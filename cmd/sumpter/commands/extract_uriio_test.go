...
// TestExtractRejectsCloudReferences asserts the edge guard still fails fast on a
// cloud *record-index* reference (the index file stays local) and on unsupported
// schemes, leaving no output behind. Cloud *source inputs* (read boundary) and
// cloud *output* (write boundary) are no longer rejected here — they route
// through the session (covered by the moto source-in / output tests).
...
func TestResolveLocalReferences(t *testing.T) {
	cases := []struct {
		...
		{"s3 output accepted (write boundary)", &ExtractOptions{OutputPath: "s3://b/out/"}, false},
