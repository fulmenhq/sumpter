...
sumpter index build CPJR01082024001607.xml \
  --selector "//SaleEvent" \
  --output CPJR.recordindex.json
```
...
**Verification:**
```bash
sumpter index verify CPJR01082024001607.xml \
  --index CPJR.recordindex.json \
  --verify-records
```
...
```yaml
# recipe.yaml
recipe_id: "cpjr_naxml_v1"
name: "CPJR Daily Sales"
record_selector: "//SaleEvent"

extraction:
  signature_config: "signature/naxml-cpjr-signature.yaml"
  extract_config: "extract/naxml-cpjr-extract.yaml"

indexes:
  default: "indexes/CPJR.recordindex.json"
  # Reference pre-built indexes for test data
```

Run with index:
```bash
sumpter recipes run extract ./recipes/cpjr \
  --input-path testdata/CPJR01082024001607.xml \
  --use-index \
  --progress
```
