...
```json
{
  ...
  "cli": {
    ...
    "argv_sanitized": ["--recipe", "recipes/pos-journal", "--out", "outputs/<run_id>"]
  },
  "recipe": {
    "id": "pos-journal-saleevent-summary",
...
- **Local FS**: `--out outputs/pricebook/` writes
  `outputs/pricebook/<run_id>/records.jsonl` and
  `outputs/pricebook/<run_id>/manifest.json`. The `<run_id>` segment is
  added automatically; tooling can list runs by directory.
- **Object store** (future): same shape with the bucket prefix
  (`s3://bucket/pricebook/<run_id>/...`).
