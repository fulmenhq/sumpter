...
async function main() {
	...
	if (args.length < 2) {
		...
		console.error(
			"Example: bun run scripts/download-sec-edgar.ts AAPL 2024-09-01 5 ~/docs/playground/public/finance/sec-edgar",
		);
		process.exit(1);
	}
	...
	console.log(
		`Downloading up to ${count} large XML files for ${ticker} around ${dateStr}`,
	);
	...
	for (const filing of filings) {
		...
		for (const xmlFile of xmlFiles) {
			if (xmlFile.size > 10000) {
				...
				console.log(
					`Downloading ${xmlFile.name} (${xmlFile.size} bytes) to ${outputPath}`,
				);
				try {
...
async function getFilings(
	...
	try {
		...
		const filingRegex =
			/<a href="\/Archives\/edgar\/data\/\d+\/([^/]+)\/[^"]*">([^<]+)<\/a>/g;
		let match: RegExpExecArray | null = filingRegex.exec(html);
		while (match !== null) {
			const accession = match[1];
			const date = match[2];
			if (date.includes(dateStr.split("-")[0])) {
				// Rough date match
				filings.push({ accession, date });
			}
			match = filingRegex.exec(html);
		}
	} catch (e) {
...
async function getXMLFiles(
	cik: string,
	filing: { accession: string; date: string },
): Promise<Array<{ name: string; url: string; size: number }>> {
	const xmlFiles: Array<{ name: string; url: string; size: number }> = [];

	try {
		// Get filing index
		const archiveCIK = cik.replace(/^0+/, "") || cik;
		const url = `https://www.sec.gov/Archives/edgar/data/${archiveCIK}/${filing.accession}/`;
		const response = await fetch(url);
		const html = await response.text();

		// Parse for XML files (simple)
		const fileRegex = /<a href="([^"]*\.xml)">([^<]+)<\/a>\s*(\d+)/g;
		let match: RegExpExecArray | null = fileRegex.exec(html);
		while (match !== null) {
			const url = match[1];
			const name = match[2];
