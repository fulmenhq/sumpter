...
- **`field_mappings[].internal: true`** (extract asset) marks a top-level scalar mapping as derive-only: computed into same-record expression scope (XPath phase first, then expressions in declaration order) but never emitted as a column and never listed in field_provenance **entries** / the portable field catalog / value_profile. Expression-only internals are allowed. Internal names must not appear in `output_schema`, `value_profile.fields`, or filter keys. Nested item/polymorphic internals are rejected. **Names are not confidential** — they still appear in recipe YAML provenance and may be referenced in emitted fields' expression lineage; do not put secrets in field names. See [extract-workflow.md](../../extract-workflow.md).
